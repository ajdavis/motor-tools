...
"""Motor benchmarking suite. Reference benchmark using bit.ly's AsyncMongo."""
...
def find(callback, asyncdb, collection, x):
    ...
    def inner_find(results, error):
        ...
        if i[0] == 0:
            ...
            i[0] -= 1
            asyncdb[collection].find({"x": x}, callback=inner_find)

    inner_find(None, None)


if __name__ == "__main__":
    trial_db = asyncmongo.Client(
        pool_id='mydb', host='127.0.0.1', port=27017, maxcached=10,
        dbname='benchmark')
